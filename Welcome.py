...
class MainWindow(tk.Tk):
    ...
    def select_columns(self):
        ...
        self.get_column_window.geometry('380x520')
        ...
        check_list = Text(top_frame, width=380, padx=60)
        check_list.pack()

        for i in range(total_columns):
            c_btn = Checkbutton(check_list, text=columns[i].upper(), variable=self.check_btn_vars[i], onvalue=1,
                                offvalue=0, font=('Helvetica', 12), bg='white')
            check_list.window_create('end', window=c_btn)
        ...
        Button(bottom_frame, default_button_options, text='Close', command=self.get_column_window.destroy).pack(side=LEFT)
